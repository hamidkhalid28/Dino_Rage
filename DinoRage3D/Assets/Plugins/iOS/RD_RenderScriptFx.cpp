...
namespace
{
  ...
  SpriteMapping *createSpriteMapping( const RD_Composition::CompositionNode *node, const RD_SpriteSheet::SpriteData *spriteData )
  {
    ...
    if ( isDeformed )
    {
      ...
      xSteps = std::max( 1u, unsigned(fabs(v2.x()-v1.x()) / MEDIUM_QUALITY_DISCRETIZATION) );
      ySteps = std::max( 1u, unsigned(fabs(v2.y()-v1.y()) / MEDIUM_QUALITY_DISCRETIZATION) );
    }
...
void RD_RenderScriptFx::update( const RD_ClipDataPtr_t &pClipData, const STD_String &projectFolder, const STD_String &sheetResolution, float frame, unsigned int color )
{
  lock();
  ...
  if ( _i->_compositions.empty() )
  {
    ...
    for ( unsigned i = 0, iEnd = (unsigned)pClipData->count() ; i<iEnd ; ++i )
    {
      ...
      if (nodeTreeView->spriteSheet(spriteSheetName))
      {
        ...
        {
          RD_Composition *composition = new RD_Composition( pSpriteSheet, nodeTreeView, pClipData->parentNodeTreeView(i) );
          _i->_compositions.push_back(composition);
    ...
    //  create vertices and indices out of composition.
    //  Create in reverse order to match Harmony rendering order. 
    for ( Impl::CompositionCol_t::const_reverse_iterator i = _i->_compositions.rbegin(), iEnd = _i->_compositions.rend() ; i!=iEnd ; ++i )
    {
      //  Create new batch.
      _i->_batches.push_back( RenderBatch() );
      RenderBatch &renderBatch = _i->_batches.back();

      //  Update batch with composition data.
      updateBatch( *i, color, renderBatch );
    }
  }
  else
  {
    //  update existing composition and create vertices and indices.
    //  Create in reverse order to match Harmony rendering order. 
    Impl::CompositionCol_t::reverse_iterator iComposition = _i->_compositions.rbegin();
    Impl::CompositionCol_t::reverse_iterator iCompositionEnd = _i->_compositions.rend();
    ...
    for ( ; iComposition != iCompositionEnd ; ++iComposition, ++iBatch )
    {
      ...
      (*iComposition)->update( frame );

      //  Update batch with composition data.
      updateBatch( *iComposition, color, *iBatch );
    }
  }
...
void RD_RenderScriptFx::cleanup()
{
  ...
  for ( Impl::CompositionCol_t::const_iterator i = _i->_compositions.begin(), iEnd = _i->_compositions.end() ; i!=iEnd ; ++i )
  {
    delete *i;
  }
  _i->_compositions.clear();
...
void RD_RenderScriptFx::updateBatch( const RD_Composition *composition, unsigned int color, RenderBatch &renderBatch )
{
  ...
  for ( RD_Composition::CompositionNodeCol_t::const_iterator i = composition->nodeBegin(), iEnd = composition->nodeEnd() ; i!=iEnd ; ++i )
  {
    ...
    if ( spriteSheet->uvs( rect, u1, v1, u2, v2 ) )
    {
      ...
      else
      {
        ...
        spriteMapping = createSpriteMapping( node, spriteData );
...
void RD_RenderScriptFx::lock()
{
  _i->_mutex.lock();
}

void RD_RenderScriptFx::unlock()
{
  _i->_mutex.unlock();
}
...
namespace
{
  ...
  typedef std::vector< BoundingVertex > BoundingVertexCol_t;
...
void RD_RenderScriptFx::calculatePolygons( BoundingVertex* &polygonArray, int &polygonSize, int* &subPolygonArray, int &subPolygonSize )
{
  ...
  Impl::CompositionCol_t::const_iterator iComposition = _i->_compositions.begin();
  Impl::CompositionCol_t::const_iterator iCompositionEnd = _i->_compositions.end();
