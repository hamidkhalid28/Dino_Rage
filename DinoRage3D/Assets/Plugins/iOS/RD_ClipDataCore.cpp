...
#define RD_ClipDataMajorVersion    (int)3
...
private:
  ...
  struct ClipData
  {
    ...
    STD_String             _parentSkeletonName;
    STD_String             _parentBoneName;
...
TV_NodeTreeViewPtr_t RD_ClipDataCore::parentNodeTreeView( unsigned idx ) const
{
  if ( idx < _i->_nodeTrees.size() )
  {
    const Impl::ClipData &data = _i->_nodeTrees[idx];
    if ( !data._parentSkeletonName.empty() && !data._parentBoneName.empty() )
    {
      for ( Impl::NodeTreeViewCol_t::iterator i = _i->_nodeTrees.begin(), iEnd = _i->_nodeTrees.end() ; i!=iEnd ; ++i )
      {
        Impl::ClipData &parentData = *i;
        if ( parentData._skeletonName.compare( data._parentSkeletonName ) == 0 )
        {
          TV_NodeTreeViewPtr_t nodeTreeView = TV_NodeTreeView::create(parentData._nodeTree);
          TV_NodeTreeViewPtr_t parentNodeTreeView = nodeTreeView->find(data._parentBoneName);
          if ( parentNodeTreeView.isValid() )
          {
            return parentNodeTreeView;
          }
        }
      }
    }
...
void RD_ClipDataCore::addNodeTree( TR_NodeTree *nodeTree, const STD_String &skeletonName, const STD_String &parentSkeletonName, const STD_String &parentBoneName )
{
  _i->_nodeTrees.push_back( Impl::ClipData() );
  Impl::ClipData &data = _i->_nodeTrees.back();

  data._skeletonName = skeletonName;
  data._parentSkeletonName = parentSkeletonName;
  data._parentBoneName = parentBoneName;
...
void RD_ClipDataCore::store( IO_PersistentStore &store ) const
{
  ...
  for ( Impl::NodeTreeViewCol_t::const_iterator i = _i->_nodeTrees.begin(), iEnd = _i->_nodeTrees.end() ; i!=iEnd ; ++i )
  {
    ...
    store << i->_skeletonName;
    store << i->_parentSkeletonName;
    store << i->_parentBoneName;
  }
...
bool RD_ClipDataCore::load( IO_PersistentStore &store )
{
  ...
  while ( nTrees-- )
  {
    ...
    STD_String skeletonName, parentSkeletonName, parentBoneName;
    store >> skeletonName;
    store >> parentSkeletonName;
    store >> parentBoneName;

    addNodeTree(nodeTree, skeletonName, parentSkeletonName, parentBoneName);
  }
