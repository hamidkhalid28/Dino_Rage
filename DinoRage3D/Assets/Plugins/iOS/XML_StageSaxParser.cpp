...
XML_SaxParserComponentPtr_t XML_StageSaxParser::startComponent(const char *nodeName, const XML_AttributeCol_t &attributes)
{
  ...
  else // if ( _state == eStage )
  {
    if ( strcmp( nodeName, kPlayTag ) == 0 )
    {
      ...
      findAttribute(attributes, kParentSkeletonTag, parentSkeletonName);
      findAttribute(attributes, kParentBoneTag, parentBoneName);

      //nodeTree->dump();
      _clip->addNodeTree(nodeTree, skeletonName, parentSkeletonName, parentBoneName );
    }
