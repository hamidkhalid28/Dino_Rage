...
RD_RenderTextureEvent::RD_RenderTextureEvent( int scriptId, int offsetx, int offsety, int width, int height, TexturePtr_t texture ) :
  _scriptId(scriptId),
  _texture(texture)
{
...
void RD_RenderTextureEvent::render( RD_Renderer *renderer )
{
  ...
  if ( pScript.isValid() )
  {
    renderer->renderScriptToTexture( pScript, _rect, _texture );
  }
}
