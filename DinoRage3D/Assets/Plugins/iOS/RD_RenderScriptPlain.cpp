...
void RD_RenderScriptPlain::update( const RD_ClipDataPtr_t &pClipData, const STD_String &projectFolder, const STD_String &sheetResolution, float frame, unsigned int color )
{
  lock();
  ...
  else
  {
    ...
    //  Create in reverse order to match Harmony rendering order. 
    for ( Impl::CompositionCol_t::reverse_iterator i = _i->_compositions.rbegin(), iEnd = _i->_compositions.rend() ; i!=iEnd ; ++i )
    {
      (*i)->update( frame );
      addBatch( *i, color );
    }
...
void RD_RenderScriptPlain::cleanup()
{
...
void RD_RenderScriptPlain::addBatch( const RD_Composition *composition, unsigned int color )
{
...
void RD_RenderScriptPlain::lock()
{
  _i->_mutex.lock();
}

void RD_RenderScriptPlain::unlock()
{
  _i->_mutex.unlock();
}
...
namespace
{
  ...
  typedef std::vector< BoundingVertexCol_t > PolygonCol_t;
